...
def nparr(im):
    a = np.fromstring(im.tostring(), dtype='uint8', count=im.width*im.height*im.nChannels)
...
def corners(homography, files):
    ...
    for i in range(len(files)):
        h, w = nparr(cv.LoadImage(files[i])).shape[0:2]
...
def main(args):
    ...
    for i in range(len(filenames) - 1):
        image1b, image2b = rgb2gray(io.imread(filenames[i])), rgb2gray(io.imread(filenames[i + 1]))
        points1, points2 = harris(image1b, count=500), harris(image2b, count=500)
        draw_corners(points1, images[i], 'corners_left')
        draw_corners(points2, images[i+1], 'corners_right')

        matches = matching(extract(image1b, (points1)), extract(image2b, (points2)))
        h = ransac(np.matrix(np.hstack((points1[matches[:,0],0:2], points2[matches[:,1],0:2]))),0.5)
        hh.append(np.linalg.inv(h[0]))
    
    hh[1] = hh[0] * hh[1]
    for i in range(len(hh)):
        midh.append(np.linalg.inv(hh[len(images)/2]) * hh[i])
    
    # Warping the images provided to the middle image
    for i in range(len(images)):
        im = nparr(cv.LoadImage(images[i]))
